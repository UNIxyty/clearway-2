...
export function renderAgentEmailText({ subject, requester, when, blocks = [], reference }) {
  ...
  for (const b of blocks) {
    ...
    else if (b.type === "section") lines.push(`${t(b.title)}:`, t(b.text), "");
    else if (b.type === "table") { for (const row of b.rows ?? []) lines.push(`  ${t(row?.[0])}: ${t(row?.[1])}`); lines.push(""); }
    else if (b.type === "mono") lines.push(t(b.title), t(b.text), "");
    else if (b.type === "verbatim") lines.push(`VERBATIM — APPROVED TEXT${b.reference ? ` (${t(b.reference)})` : ""}:`, t(b.text), t(b.by), "");
    else if (b.type === "callout") lines.push(`! ${t(b.title)} ${t(b.text)}`, "");
