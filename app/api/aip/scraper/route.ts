...
const AIP_SCRAPER_PREFIX = "aip/scraper";
...
async function getFromS3(icao: string): Promise<{ airports: unknown[]; updatedAt: string | null } | null> {
  ...
  try {
    ...
    const key = `${AIP_SCRAPER_PREFIX}/${icao}.json`;
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = (await res.json().catch(() => ({}))) as { airports?: unknown[]; error?: string; detail?: string; code?: number };
    if (!res.ok) {
      return NextResponse.json(
    ...
    return NextResponse.json({ error: "AIP sync request failed", detail: msg }, { status: 502 });
  }
}
