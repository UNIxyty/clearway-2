...
export function ResultsView({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className={embedded ? '' : 'min-h-screen bg-page'}>
      {/* Header — title hidden when embedded (console owns it); round tabs stay. */}
      <div className={embedded ? '' : 'bg-white border-b border-black/[0.07]'}>
        ...
        {/* Round tabs */}
        <div className={`${embedded ? '' : 'max-w-5xl mx-auto'} px-5 flex gap-0.5 overflow-x-auto`}>
          {ROUND_TABS.map(t => (
            <button
              key={t.id}
      ...
      {/* Match cards */}
      <main className={embedded ? 'pt-5 space-y-4' : 'max-w-5xl mx-auto px-5 py-6 space-y-4'}>
        {currentIds.map(code => {
          const row = results[code];
