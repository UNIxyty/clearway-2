...
async function fetchStatus(countryKey: string, sessionId: string): Promise<StatusPayload | null> {
  try {
    const res = await fetch("/api/blocked-hitl-vnc", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "status", country: countryKey, sessionId }),
    });
...
export function HitlCountryViewerClient({
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      ...
      const data = await fetchStatus(countryKey, sessionId);
      if (!data) return;
  ...
  }, [countryKey, sessionId, shouldPoll]);
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {autoCloseNotice && (
              <div
                className="rounded-lg border border-green-500/40 bg-green-500/10 px-3 py-2 text-sm text-green-900 dark:text-green-200"
                aria-live="polite"
              >
                {autoCloseNotice}
              </div>
            ...
            <div className="overflow-hidden rounded-xl border bg-card">
              {hasValidViewer ? (
                <iframe
                  src={noVncUrl}
                  title={`${countryName} noVNC viewer`}
                ...
                <div className="flex min-h-[52dvh] items-center justify-center px-6 py-10 text-center">
                  <div className="max-w-md space-y-3">
                    <MonitorIcon className="mx-auto size-8 text-muted-foreground" aria-hidden="true" />
                    <p className="font-medium text-foreground">Viewer is not ready</p>
                    <p className="text-pretty text-sm text-muted-foreground">
                      Start a session from the {countryName} HITL page first. The viewer opens automatically with a
                      valid noVNC URL.
                    </p>
                  </div>
                </div>
