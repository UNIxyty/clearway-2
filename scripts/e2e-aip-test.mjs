#!/usr/bin/env node
/**
 * AIP UI Debug Test — EAD airports only.
 *
 * Same structure and output shape as e2e-portal-test.mjs so that
 * generate-test-report.mjs and send-test-webhook.mjs work unchanged.
 *
 * What it checks per airport (fast — does NOT wait for full sync to finish):
 *   - pageLoad : AIP (EAD) heading appears after search
 *   - aip      : "Syncing AIP from server" or "Loading AIP" text appears within 10s of clicking sync
 *   - map      : skipped (marked ⏭️ in report)
 *   - notams   : skipped
 *   - gen      : skipped
 ...
 *
 * Env vars (same as e2e-portal-test.mjs):
 *   PORTAL_URL, HEADLESS, DISABLE_AUTH_FOR_TESTING, DISABLE_AI_FOR_TESTING,
 *   MAX_AIRPORTS, COUNTRY_FILTER, TEST_RESULTS_DIR,
 *   PLAYWRIGHT_STORAGE_STATE_PATH, TEST_USER_EMAIL, MAGIC_LINK_URL,
 *   N8N_WEBHOOK_URL / WEBHOOK_URL, AWS_S3_BUCKET, AWS_REGION,
 *   E2E_REPORTS_S3_PREFIX, E2E_REPORT_URL_EXPIRES_IN
 */
...
// Fetch all countries but keep only those that have at least one EAD airport.
...
// ── Auth (identical to e2e-portal-test.mjs) ──────────────────────────────────
...
// ── Per-airport test logic ────────────────────────────────────────────────────

async function runOneAirport(page, airport, country, screenshotRoot) {
  const icao = String(airport?.icao || "").toUpperCase();

  // Output shape matches e2e-portal-test.mjs exactly so generate-test-report works unchanged.
  const result = {
    icao,
    name: airport?.name || "",
    country,
    isEad: true, // only EAD airports reach this function
    startedAt: nowIso(),
    checks: {
      pageLoad:   { pass: false },
      map:        { pass: false, hasCoords: false, skipped: true },
      notams:     { pass: false, skipped: true },
      aip:        { pass: false, skippedAi: DISABLE_AI_FOR_TESTING },
      gen:        { pass: false, skipped: true, skippedAi: true },
      screenshot: { pass: false, path: "" },
    },
    errors: [],
  };

  // ── 1. Page load: search ICAO and wait for AIP (EAD) heading ──────────────
  try {
    await page.goto(PORTAL_URL, { waitUntil: "load" });
  ...
  // ── 2. AIP loading UI check (fast — does NOT wait for sync to finish) ─────
...
async function main() {
  ...
  // Output structure matches e2e-portal-test.mjs exactly.
  const output = {
    startedAt: nowIso(),
    portalUrl: PORTAL_URL,
    testType: "aip-ui-debug",
  ...
  for (const { country, airports } of countries) {
    ...
    for (const airport of airports) {
      ...
      const aipPass = airportResult.checks.aip.pass;
      console.log(`       ${aipPass ? "PASS" : "FAIL"}${airportResult.errors.length ? " — " + airportResult.errors[0] : ""}`);
      countryResult.airports.push(airportResult);
      // Write incrementally so results are not lost if the run is interrupted.
  ...
  for (const country of output.countries) {
    for (const airport of country.airports) {
      // PASS = pageLoad + aip both pass (map/notams/gen are skipped, not counted as failures).
      const allPass = airport.checks.pageLoad.pass && airport.checks.aip.pass && airport.checks.screenshot.pass;
  ...
  console.log(`\nE2E AIP debug run complete. Raw results: ${rawOutputPath}`);
  ...
  // ── Generate report and send webhook (same as full E2E pipeline) ──────────
  console.log("\nGenerating report...");
  try {
    execFileSync(process.execPath, ["scripts/generate-test-report.mjs"], {
      stdio: "inherit",
