...
function checkMark(pass, skipped = false) {
  if (skipped) return "⏭️";
  return pass ? "✅" : "❌";
}
...
function buildMarkdown(data, reportPath) {
  const lines = [];
  lines.push("# Portal E2E Test Report");
  lines.push("");
  ...
  for (const country of data.countries || []) {
    ...
    for (const airport of country.airports || []) {
      ...
      lines.push(`- Map Loaded + Location: ${checkMark(c.map?.pass)}${c.map?.hasCoords ? " (coords available)" : " (coords missing/failed)"}`);
      lines.push(`- NOTAMs Loaded: ${checkMark(c.notams?.pass, c.notams?.skipped)}`);
      lines.push(`- AIP Loaded: ${checkMark(c.aip?.pass)}${c.aip?.skippedAi ? " (AI skipped)" : ""}`);
      lines.push(`- GEN Loaded: ${checkMark(c.gen?.pass, c.gen?.skipped)}${c.gen?.skippedAi ? " (AI skipped)" : ""}`);
...
function main() {
  ...
  const markdown = buildMarkdown(data, reportPath);
