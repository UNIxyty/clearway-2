 ...
 *
 * Usage: node scripts/skylink-weather.mjs [--json] <ICAO>
 */
...
function extractWeatherText(payload) {
  if (!payload) return "";
...
async function main() {
  ...
  if (!/^[A-Z0-9]{4}$/.test(icao)) {
    console.error("Usage: node scripts/skylink-weather.mjs [--json] <ICAO>");
    process.exit(1);
  }

  progress(`Fetching METAR from SkyLink API for ${icao}`);
  const response = await fetchSkylinkWeather(icao);
  const weatherText = extractWeatherText(response);
  const payload = {
    icao,
    weather: weatherText,
