...
async function resolveNewestPackageMenu() {
  ...
  return { version, menuUrl, menuHtml };
}

async function downloadPdfToFile(pdfUrl, outFile, label) {
  const pdfRes = await fetch(pdfUrl);
  if (!pdfRes.ok) throw new Error(`${label} PDF fetch failed: ${pdfRes.status} ${pdfRes.statusText}`);
...
`);
  ...
  const downloadAd2Arg = process.argv.indexOf("--download-ad2");
  const ad2Icao = downloadAd2Arg >= 0 ? String(process.argv[downloadAd2Arg + 1] || "").trim().toUpperCase() : "";
  ...
  if (downloadGen12) {
    const { version, menuUrl, menuHtml } = await resolveNewestPackageMenu();
    const entries = parseGenEntries(menuHtml);
    const gen12 = entries.find((e) => String(e.anchor).toUpperCase() === "GEN-1.2");
    if (!gen12) throw new Error("GEN 1.2 not found in Bahrain menu.");
    const htmlUrl = new URL(gen12.href, menuUrl).href;
    const pdfUrl = htmlToPdfUrl(htmlUrl);
    mkdirSync(OUT_GEN, { recursive: true });
    const outFile = join(OUT_GEN, safeFilename(`${version.label}_${gen12.anchor}.pdf`));
    await downloadPdfToFile(pdfUrl, outFile, "GEN");
    console.error(`[BH] saved GEN 1.2: ${outFile}`);
    return;
  }

  if (/^[A-Z0-9]{4}$/.test(ad2Icao)) {
    const { version, menuUrl, menuHtml } = await resolveNewestPackageMenu();
    const airports = parseAd2Icaos(menuHtml);
    const chosen = airports.find((a) => a.icao === ad2Icao);
    if (!chosen) throw new Error(`AD 2 ICAO not found in Bahrain menu: ${ad2Icao}`);
    ...
    await downloadPdfToFile(pdfUrl, outFile, "AD2");
    console.error(`[BH] saved AD2 ${ad2Icao}: ${outFile}`);
    return;
  }

  if (collectMode()) {
    try {
      const { version, menuHtml } = await resolveNewestPackageMenu();
      const airports = parseAd2Icaos(menuHtml);
      printCollectJson({
