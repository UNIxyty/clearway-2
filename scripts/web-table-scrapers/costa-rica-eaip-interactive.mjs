#!/usr/bin/env node
/**
 * Interactive Costa Rica eAIP downloader.
 *
 * Usage:
 *   node scripts/web-table-scrapers/costa-rica-eaip-interactive.mjs
 *   node scripts/web-table-scrapers/costa-rica-eaip-interactive.mjs --insecure
 *   node scripts/web-table-scrapers/costa-rica-eaip-interactive.mjs --collect
 */
...
const OUT_GEN = join(PROJECT_ROOT, "downloads", "costa-rica-eaip", "GEN");
const OUT_AD2 = join(PROJECT_ROOT, "downloads", "costa-rica-eaip", "AD2");

const HISTORY_URL = "https://www.cocesna.org/aipca/AIPMR/inicio.html";
...
async function fetchText(url) {
  ...
  try {
    const res = await fetch(url, {
      ...
      headers: { "User-Agent": "Mozilla/5.0 (compatible; clearway-cr-scraper/1.0)" },
    });
...
async function downloadPdfFromCandidates(candidates, outFile) {
  ...
  for (const url of candidates) {
    try {
      const res = await fetch(url, { headers: { "User-Agent": "Mozilla/5.0 (compatible; clearway-cr-scraper/1.0)" } });
...
async function main() {
  if (process.argv.includes("--help") || process.argv.includes("-h")) {
    console.log(`Usage: node scripts/web-table-scrapers/costa-rica-eaip-interactive.mjs [--insecure] [--collect]
       node scripts/web-table-scrapers/costa-rica-eaip-interactive.mjs --download-ad2 <ICAO>
       node scripts/web-table-scrapers/costa-rica-eaip-interactive.mjs --download-gen12`);
    return;
  }
  if (process.argv.includes("--insecure")) {
    process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";
    console.error("[CR] TLS verification disabled (--insecure)\n");
  }
  ...
  if (collectMode()) {
    try {
      ...
      console.error("[CR] collect failed:", err?.message || err);
      process.exit(1);
    } finally {
      await closeBrowser();
    }
  ...
  let rl = null;
  try {
    console.error("Costa Rica eAIP — interactive downloader\n");
    const historyHtml = await fetchText(HISTORY_URL);
    const issues = parseIssues(historyHtml);
    if (!issues.length) throw new Error("No issue links found.");
    ...
    if (downloadAd2Icao) {
      ...
      if (!chosen) throw new Error(`AD2 ICAO not found in Costa Rica menu: ${downloadAd2Icao}`);
...
main().catch((err) => {
  console.error("[CR] failed:", err?.message || err);
  process.exit(1);
});
