#!/usr/bin/env node
/**
 * Interactive Estonia eAIP downloader.
 *
 * Source:
 * - https://aim.eans.ee/en/eaip
 */
...
const OUT_GEN = join(PROJECT_ROOT, "downloads", "estonia-eaip", "GEN");
const OUT_AD2 = join(PROJECT_ROOT, "downloads", "estonia-eaip", "AD2");
const ENTRY_URL = "https://aim.eans.ee/en/eaip";
const UA = "Mozilla/5.0 (compatible; clearway-estonia-eaip/1.0)";
const FETCH_TIMEOUT_MS = 30_000;
const log = (...args) => console.error("[ESTONIA]", ...args);
...
function parseCurrentIssueUrl(entryHtml) {
  const links = [...String(entryHtml || "").matchAll(/href=["']([^"']*eaip\.eans\.ee\/20\d{2}-\d{2}-\d{2}\/html\/index-en-GB\.html)["']/gi)].map(
    (m) => m[1],
  );
  if (!links.length) return "";
  links.sort((a, b) => String(b).localeCompare(String(a)));
  return links[0];
}

function parseEffectiveDateFromUrl(url) {
...
function parseMenuUrl(frameSetHtml, frameSetUrl) {
  ...
  if (!href) throw new Error("Could not resolve Estonia menu frame URL.");
...
function parseGenEntries(menuHtml, menuUrl) {
  ...
  for (const m of String(menuHtml || "").matchAll(/href=["']([^"']*EE-GEN-([0-9.]+)-en-GB\.html(?:#[^"']*)?)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
...
function parseAd2Entries(menuHtml, menuUrl) {
  ...
  for (const m of String(menuHtml || "").matchAll(/href=["']([^"']*EE-AD-2\.([A-Z0-9]{4})-en-GB\.html(?:#[^"']*)?)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
...
async function resolveContext() {
  ...
  const indexUrl = parseCurrentIssueUrl(entryHtml);
  if (!indexUrl) throw new Error("Could not resolve currently effective Estonia eAIP issue URL.");
  const indexHtml = await fetchText(indexUrl);
  const frameSetUrl = parseFrameSetUrl(indexHtml, indexUrl);
  const frameSetHtml = await fetchText(frameSetUrl);
  const menuUrl = parseMenuUrl(frameSetHtml, frameSetUrl);
  const menuHtml = await fetchText(menuUrl);
  const effectiveDate = parseEffectiveDateFromUrl(indexUrl);
