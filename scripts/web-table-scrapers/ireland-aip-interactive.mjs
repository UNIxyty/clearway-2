#!/usr/bin/env node
/**
 * Interactive Ireland AIP downloader.
 *
 * Source:
...
const OUT_GEN = join(PROJECT_ROOT, "downloads", "ireland-aip", "GEN");
const OUT_AD2 = join(PROJECT_ROOT, "downloads", "ireland-aip", "AD2");
const ENTRY_URL =
  "https://www.airnav.ie/air-traffic-management/aeronautical-information-management/aip-package#Part_III_-_Aerodromes_(AD)";
const UA = "Mozilla/5.0 (compatible; clearway-ireland-aip/1.0)";
const log = (...args) => console.error("[IRELAND]", ...args);
...
function parseContext(entryHtml) {
  const links = [...new Set(parsePdfLinks(entryHtml))];
  const genUrl = links.find((u) => /EI[_-]?GEN[_-]?1[_-]?2/i.test(u)) || "";
  const byIcao = new Map();
  for (const u of links) {
    const m = u.match(/EI[_-]?AD[_-]?2[_-]?([A-Z0-9]{4})/i);
    if (!m) continue;
...
async function resolveContext() {
  ...
  const parsed = parseContext(entryHtml);
  ...
  if (!parsed.genUrl) throw new Error("GEN 1.2 PDF link not found.");
  if (!parsed.ad2Entries.length) throw new Error("No AD2 PDFs found.");
  return {
    effectiveDate,
    genUrl: parsed.genUrl,
    ad2Entries: parsed.ad2Entries,
  };
}
