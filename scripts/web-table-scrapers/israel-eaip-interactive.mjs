...
async function main() {
  if (process.argv.includes("--help") || process.argv.includes("-h")) {
    console.log(`Usage: node scripts/web-table-scrapers/israel-eaip-interactive.mjs [--insecure] [--collect]
       node scripts/web-table-scrapers/israel-eaip-interactive.mjs --download-ad2 <ICAO>
       node scripts/web-table-scrapers/israel-eaip-interactive.mjs --download-gen12`);
    return;
  }
  ...
  try {
    ...
    if (downloadAd2Icao) {
      ...
      if (!chosen) throw new Error(`AD2 ICAO not found in Israel menu: ${downloadAd2Icao}`);
      const pdfUrl = htmlToPdfUrl(chosen.htmlUrl);
