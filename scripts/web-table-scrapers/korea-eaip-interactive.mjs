#!/usr/bin/env node
/**
 * Interactive Republic of Korea eAIP downloader.
 *
 * Source:
 * - https://aim.koca.go.kr/eaipPub/Package/history-en-GB.html
 *
 * Usage:
 *   node scripts/web-table-scrapers/korea-eaip-interactive.mjs
 *   node scripts/web-table-scrapers/korea-eaip-interactive.mjs --insecure
 *   node scripts/web-table-scrapers/korea-eaip-interactive.mjs --collect
 */
...
const OUT_GEN = join(PROJECT_ROOT, "downloads", "korea-eaip", "GEN");
const OUT_AD2 = join(PROJECT_ROOT, "downloads", "korea-eaip", "AD2");
...
async function main() {
  if (process.argv.includes("--help") || process.argv.includes("-h")) {
    console.log(`Usage: node scripts/web-table-scrapers/korea-eaip-interactive.mjs [--insecure] [--collect]
       node scripts/web-table-scrapers/korea-eaip-interactive.mjs --download-ad2 <ICAO>
       node scripts/web-table-scrapers/korea-eaip-interactive.mjs --download-gen12`);
    return;
  }
  if (process.argv.includes("--insecure")) {
    process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";
    console.error("[KR] TLS verification disabled (--insecure)\n");
  }
  ...
  if (collectMode()) {
    try {
      ...
      console.error("[KR] collect failed:", err?.message || err);
      process.exit(1);
    }
    return;
  }
  ...
  let rl = null;
  try {
    console.error("Korea eAIP — interactive downloader\n");
    const historyHtml = await fetchText(HISTORY_URL);
    const issues = parseIssues(historyHtml);
    if (!issues.length) throw new Error("No effective-date issues found.");
    ...
    if (downloadAd2Icao) {
      ...
      if (!chosen) throw new Error(`AD2 ICAO not found in Korea menu: ${downloadAd2Icao}`);
...
main().catch((err) => {
  console.error("[KR] failed:", err?.message || err);
  process.exit(1);
});
