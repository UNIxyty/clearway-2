#!/usr/bin/env node
/**
 * Interactive Kosovo eAIP downloader.
 *
 * Usage:
 *   node scripts/web-table-scrapers/kosovo-eaip-interactive.mjs
 *   node scripts/web-table-scrapers/kosovo-eaip-interactive.mjs --insecure
 *   node scripts/web-table-scrapers/kosovo-eaip-interactive.mjs --collect
 */
...
const OUT_GEN = join(PROJECT_ROOT, "downloads", "kosovo-eaip", "GEN");
const OUT_AD2 = join(PROJECT_ROOT, "downloads", "kosovo-eaip", "AD2");

// Official caa.rks-gov page currently carries dead links; ASHNA mirror is live.
const HISTORY_URL = "https://www.ashna-ks.org/eAIP/default.html";
...
async function fetchText(url) {
  ...
  try {
    const res = await fetch(url, {
      ...
      headers: { "User-Agent": "Mozilla/5.0 (compatible; clearway-xk-scraper/1.0)" },
    });
...
async function downloadPdfFromCandidates(candidates, outFile) {
  ...
  for (const url of candidates) {
    try {
      const res = await fetch(url, { headers: { "User-Agent": "Mozilla/5.0 (compatible; clearway-xk-scraper/1.0)" } });
...
async function main() {
  if (process.argv.includes("--help") || process.argv.includes("-h")) {
    console.log(`Usage: node scripts/web-table-scrapers/kosovo-eaip-interactive.mjs [--insecure] [--collect]
       node scripts/web-table-scrapers/kosovo-eaip-interactive.mjs --download-ad2 <ICAO>
       node scripts/web-table-scrapers/kosovo-eaip-interactive.mjs --download-gen12`);
    return;
  }
  if (process.argv.includes("--insecure")) {
    process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";
    console.error("[XK] TLS verification disabled (--insecure)\n");
  }
  ...
  if (collectMode()) {
    try {
      ...
      console.error("[XK] collect failed:", err?.message || err);
      process.exit(1);
    }
    return;
  }
  ...
  let rl = null;
  try {
    console.error("Kosovo eAIP — interactive downloader\n");
    const historyHtml = await fetchText(HISTORY_URL);
    const issues = parseIssues(historyHtml);
    if (!issues.length) throw new Error("No issue links found.");
    ...
    if (autoMode) {
      ...
      console.error(`Auto-selected newest issue: ${issue.issueCode || issue.label}`);
    } else {
    ...
    if (downloadAd2Icao) {
      ...
      if (!chosen) throw new Error(`AD2 ICAO not found in Kosovo menu: ${downloadAd2Icao}`);
      const candidates = buildPdfCandidates(chosen.htmlUrl);
...
main().catch((err) => {
  console.error("[XK] failed:", err?.message || err);
  process.exit(1);
});
