...
async function main() {
  if (process.argv.includes("--help") || process.argv.includes("-h")) {
    console.log(`Usage: node scripts/web-table-scrapers/mongolia-eaip-interactive.mjs [--insecure] [--collect]
       node scripts/web-table-scrapers/mongolia-eaip-interactive.mjs --download-ad2 <ICAO>
       node scripts/web-table-scrapers/mongolia-eaip-interactive.mjs --download-gen12`);
    return;
  }
  ...
  try {
    ...
    let issue;
    if (autoMode) {
      issue = pickNewestIssueByIso(issues, (x) => `${x.issueCode} ${x.label}`);
    ...
    if (downloadAd2Icao) {
      ...
      if (!chosen) throw new Error(`AD2 ICAO not found in Mongolia menu: ${downloadAd2Icao}`);
