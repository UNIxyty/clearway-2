 ...
 *
 * Usage:
 *   node scripts/web-table-scrapers/rwanda-aip-interactive.mjs
 *   node scripts/web-table-scrapers/rwanda-aip-interactive.mjs --insecure
 *   node scripts/web-table-scrapers/rwanda-aip-interactive.mjs --collect
 */
...
import { createAsecnaFetch, parseAsecnaCli } from "../asecna/asecna-eaip-http.mjs";
...
async function main() {
  ...
  if (collectMode()) {
    try {
      ...
      printCollectJson({ effectiveDate: null, ad2Icaos: parseRwandaAd2Icaos(menuHtml) });
    } catch (err) {
      console.error("[RWANDA] collect failed:", err?.message || err);
      process.exit(1);
    }
    return;
  }
