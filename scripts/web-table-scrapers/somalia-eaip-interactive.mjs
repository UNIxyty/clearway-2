#!/usr/bin/env node
/**
 * Interactive Somalia eAIP downloader.
 *
 * Source:
 * - https://aip.scaa.gov.so/history-en-GB.html
 *
 * Usage:
 *   node scripts/web-table-scrapers/somalia-eaip-interactive.mjs
 *   node scripts/web-table-scrapers/somalia-eaip-interactive.mjs --insecure
 *   node scripts/web-table-scrapers/somalia-eaip-interactive.mjs --collect
 */
...
const OUT_GEN = join(PROJECT_ROOT, "downloads", "somalia-eaip", "GEN");
const OUT_AD2 = join(PROJECT_ROOT, "downloads", "somalia-eaip", "AD2");

const HISTORY_URL = "https://aip.scaa.gov.so/history-en-GB.html";
const FETCH_TIMEOUT_MS = 30_000;
const UA = "Mozilla/5.0 (compatible; clearway-so-scraper/1.0)";
...
async function main() {
  if (process.argv.includes("--help") || process.argv.includes("-h")) {
    console.log(`Usage: node scripts/web-table-scrapers/somalia-eaip-interactive.mjs [--insecure] [--collect]
       node scripts/web-table-scrapers/somalia-eaip-interactive.mjs --download-ad2 <ICAO>
       node scripts/web-table-scrapers/somalia-eaip-interactive.mjs --download-gen12`);
    return;
  }
  if (process.argv.includes("--insecure")) {
    process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";
    console.error("[SO] TLS verification disabled (--insecure)\n");
  }
  ...
  if (collectMode()) {
    try {
      ...
      console.error("[SO] collect failed:", err?.message || err);
      process.exit(1);
    }
    return;
  }
  ...
  let rl = null;
  try {
    console.error("Somalia eAIP — interactive downloader\n");
    const historyHtml = await fetchText(HISTORY_URL);
    const issues = parseIssues(historyHtml);
    if (!issues.length) throw new Error("No effective-date issues found.");
...
main().catch((err) => {
  console.error("[SO] failed:", err?.message || err);
  process.exit(1);
});
