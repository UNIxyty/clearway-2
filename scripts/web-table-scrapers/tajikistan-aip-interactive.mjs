#!/usr/bin/env node
/**
 * Interactive Tajikistan AIP downloader.
 *
 * Flow implemented exactly as site navigation:
 * 1) Open AIP Republic of Tajikistan page
 * 2) Open "AIP Republic of Tajikistan" (validaip)
 * 3) Choose "English"
 * 4) Parse menueng.htm (known legacy menu format with ItemLink(...))
 *
 * Source:
 * - http://www.caica.ru/aiptjk/?lang=en
 *
 * Usage:
...
const OUT_GEN = join(PROJECT_ROOT, "downloads", "tajikistan-aip", "GEN");
const OUT_AD2 = join(PROJECT_ROOT, "downloads", "tajikistan-aip", "AD2");
...
function parseValidaipUrl(landingHtml) {
  ...
  if (!m?.[1]) throw new Error('Could not find "AIP Republic of Tajikistan" button target.');
...
function parseAd2Entries(itemLinks) {
  ...
  for (const [icao, docs] of byIcao.entries()) {
    // Prefer the core AD2 text/data page if present, else first available PDF.
    const preferred =
...
async function main() {
  ...
  let rl = null;
  try {
    console.error("Tajikistan AIP — interactive downloader\n");
...
main().catch((err) => {
  console.error("[TJ] failed:", err?.message || err);
  process.exit(1);
});
