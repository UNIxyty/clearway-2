...
async function main() {
  if (process.argv.includes("--help") || process.argv.includes("-h")) {
    console.log(`Usage: node scripts/web-table-scrapers/uae-eaip-interactive.mjs [--insecure] [--collect]
       node scripts/web-table-scrapers/uae-eaip-interactive.mjs --download-ad2 <ICAO>
       node scripts/web-table-scrapers/uae-eaip-interactive.mjs --download-gen12`);
    return;
  }
    ...
    if (downloadGen12 || downloadAd2Icao) {
      ...
      if (downloadGen12) {
        ...
        if (
          ...
          console.error("[UAE] GEN 1.2 not found; falling back to first available GEN entry.");
        }
        mkdirSync(OUT_GEN, { recursive: true });
        const outFile = join(OUT_GEN, "OM-GEN-1.2.pdf");
      ...
      if (!chosen) throw new Error(`AD2 ICAO not found in UAE package: ${downloadAd2Icao}`);
